# Hi 👋, I'm Fenil Bhingradiya

### Aspiring Software Engineer | Full-Stack Web Developer | C++ & DSA

I'm a Computer Science student passionate about building web applications,
solving problems with C++, and continuously improving my software development skills.

### 🚀 About Me

- 💻 Full-Stack Web Developer
- 🌐 Experienced with MERN & MEAN Stack
- 🧠 Currently focused on C++ & Data Structures and Algorithms
- 🚀 Building real-world projects to strengthen my development skills
- 📚 Continuously learning new technologies
- 🤝 Open to internships, collaborations, and software development opportunities

### 🛠️ Tech Stack

**Languages**
- C++
- Java
- JavaScript
- HTML
- CSS

**Web Development**
- React.js
- Node.js
- Express.js
- MongoDB
- Angular

**Tools & Technologies**
- Git
- GitHub
- REST APIs
- MySQL
- Oracle

### 📌 Featured Projects

#### 🛍️ ThreadTheory
A full-stack clothing e-commerce web application built using the MERN stack.

**Tech:** MongoDB • Express.js • React.js • Node.js

#### ⌚ Watchera
A web application related to branded watches developed using the MEAN stack.

**Tech:** MongoDB • Express.js • Angular • Node.js

### 🧩 100 Days of C++ & DSA

Currently working through a structured 100-day journey focused on:

- C++ Programming
- Data Structures
- Algorithms
- Problem Solving
- LeetCode Practice

### 🏆 Certifications & Achievements

- Generative AI
- Distributed Version Control System using Git
- KickStart-2026 Programming Competition (CMPICA, Charusat)

### 📈 GitHub Journey

I believe in learning by building, solving problems consistently,
and improving one step at a time.

### 🤝 Connect With Me

- LinkedIn: [linkedin.com/in/fenil-bhingradiya](https://www.linkedin.com/in/fenil-bhingradiya-95253b428/)
- Email: fenilbhingradiya150@gmail.com
